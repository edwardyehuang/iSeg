...
def evaluate (distribute_strategy,
              model, 
              data, 
              batch_size,
              num_class, 
    ...
    with distribute_strategy.scope():
        ...
        loss_func = catecrossentropy_ignore_label_loss(num_class = num_class, 
        ...
        with tqdm(total = val_image_count) as pbar:
            for inputs in ds:
                ...
                pbar.set_description("Processed : {:}, current loss = {:4f}, current IOU = {:.2f} %".format(processed_count,
                    loss_metrics.result(),
                    iou_metrics.result().numpy() * 100))
...
def eval_step (ds_inputs,
    ...
    def step_fn (inputs):
        ...
        predictions = model.inference_with_multi_scales(images, training = False,
