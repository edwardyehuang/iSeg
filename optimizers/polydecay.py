...
class WarmUpPolyDecay (tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __init__(self, initial_learning_rate, decay_steps, end_learning_rate = 0.0001, warmup_steps = 0, warmup_learning_rate = 1e-4, power = 1.0, name = None):
    ...
    def __call__(self, step):
        with tf.name_scope(self.name or "WarmUpPolyDecay") as name:
            ...
            learning_rate = tf.add(tf.math.multiply(initial_learning_rate - end_learning_rate, tf.math.pow(1 - p, power)), end_learning_rate, name = name)
