...
def inference_fn(inputs, model, num_class = 21,
                                training = False, 
    ...
    if sliding_window_crop_size is None:
        ...
        model_results = inference_with_sliding_window(inputs, num_class = num_class, 
                                                              model = model,
                                                              training = training,
                                                              windows_size = sliding_window_crop_size)
...
def get_sliding_window_slices_paddings_list (stride_h, stride_w, inputs_height, inputs_width):
    ...
    slices_list = tf.TensorArray(tf.int32, size = total_sliding_indexs_len, dynamic_size = False, clear_after_read = False, name = "slices_list")
    paddings_list = tf.TensorArray(tf.int32, size = total_sliding_indexs_len, dynamic_size = False, clear_after_read = False, name = "paddings_list")
    ...
    return (slices_list_result, 
            paddings_list_result, 
            inference_count_map)



def inference_with_sliding_window (inputs, model, num_class = 21, 
                                                  training = False, 
    ...
    slices_list, paddings_list, inference_count_map = get_sliding_window_slices_paddings_list(stride_h, stride_w, inputs_height, inputs_width)
    ...
    def loop_body (i, results = None):
        ...
        if results is None:
            results = create_base_tensor_for_cropped_result(cropped_results,
                                                            (inputs_height, inputs_width))


        cropped_results = multi_results_handler(cropped_results,
