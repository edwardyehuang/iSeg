...
class MobileNetV2 (tf.keras.Model):
    ...
    def __init__(self, alpha = 1.0, return_endpoints = False, name = None):
        ...
        self.conv1 = tf.keras.layers.Conv2D(first_block_filters, (3, 3), strides = (2, 2), padding = "same", use_bias = False, name = "Conv1")
    ...
    def __add_blocks (self, filters, alpha, stride = 1, expansion = 1, repeated = 1):
        ...
        for i in range(repeated):
            ...
            self.blocks.append(InvertedResBlock(filters = filters, alpha = alpha, stride = adjusted_stride, expansion = expansion, block_id = block_id))
...
class InvertedResBlock (tf.keras.Model):
    ...
    def __init__(self, expansion, stride, alpha, filters, block_id):
        ...
        self.depthwise = tf.keras.layers.DepthwiseConv2D((3, 3), strides = self.__stride,
                                                                 use_bias = False, 
    ...
    def build(self, input_shape):
        ...
        if self.block_id:
            self.expand_conv = tf.keras.layers.Conv2D(self.expansion * self.in_channels, (1, 1), padding = "same", 
                                                                                                 use_bias = False, 
        ...
        self.project = tf.keras.layers.Conv2D(self.pointwise_filters, (1, 1), padding = "same", use_bias = False, name = self.prefix + "project")
