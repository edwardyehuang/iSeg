...
class Stack (tf.keras.Model):
    def __init__ (self, filters, blocks_count, stride1 = 2,
                                               use_bias = True, 
                                               norm_method = None,
                                               custom_block = None,
                                               name = None):
        ...
        self.blocks.append(block_func(filters, stride = stride1, use_bias = use_bias, norm_method = norm_method, name = name + "_block1"))

        for i in range (2, blocks_count + 1):
            block = block_func(filters, conv_shortcut = False, use_bias = use_bias, norm_method = norm_method, name = name + "_block" + str(i))
...
class Stack2 (tf.keras.Model):
    def __init__ (self, filters, blocks_count, stride1 = 2, 
        ...
        self.blocks.append(block_func(filters, stride = 1, conv_shortcut = True, use_bias = use_bias, norm_method = norm_method, name = name + "_block1"))
        
        for i in range (2, blocks_count):
            block = block_func(filters, conv_shortcut = False, use_bias = use_bias, norm_method = norm_method, name = name + "_block" + str(i))
            self.blocks.append(block)

        self.blocks.append(block_func(filters, stride = stride1, 
                                               conv_shortcut = False, 
                                               use_bias = use_bias, 
...
class ResNet (tf.keras.Model):
    def __init__(self, stacks, use_bias = True, norm_method = None, replace_7x7_conv = False, return_endpoints = False, name = "resnet"):
        ...
        self.poo1_pool = tf.keras.layers.MaxPooling2D(3, strides = 2,
                                                         padding  = "same" if replace_7x7_conv else "valid", 
                                                         name ="pool1_pool")
    ...
    def build_3x3_resnet(self, use_bias = True, norm_method = None):
        ...
        self.conv1_1_conv = tf.keras.layers.Conv2D(64, 3, strides = 2, padding = "SAME", use_bias = use_bias, name = "conv1_1_conv")
        self.conv1_1_bn = normalization(epsilon = BN_EPSILON, method = norm_method, name = "conv1_1_bn")

        self.conv1_2_conv = tf.keras.layers.Conv2D(64, 3, strides = 1, padding = "SAME", use_bias = use_bias, name = "conv1_2_conv")
        self.conv1_2_bn = normalization(epsilon = BN_EPSILON, method = norm_method, name = "conv1_2_bn")

        self.conv1_3_conv = tf.keras.layers.Conv2D(128, 3, strides = 1, padding = "SAME", use_bias = use_bias, name = "conv1_3_conv")
...
def resnet50 (use_bias = True, norm_method = None, replace_7x7_conv = False, slim_behaviour = False, custom_block = None, return_endpoints = False):

    return get_resnet(resnet_name = ss.RESNET50,
                      use_bias = use_bias,
                      norm_method = norm_method,
...
def resnet101(use_bias = True, norm_method = None, replace_7x7_conv = False, slim_behaviour = False, custom_block = None, return_endpoints = False):

    return get_resnet(resnet_name = ss.RESNET101,
                      use_bias = use_bias,
                      norm_method = norm_method,
                      replace_7x7_conv = replace_7x7_conv,
                      slim_behaviour = slim_behaviour,
...
def resnet152(use_bias = True, norm_method = None, replace_7x7_conv = False, slim_behaviour = False, custom_block = None, return_endpoints = False):

    return get_resnet(resnet_name = ss.RESNET152,
                      use_bias = use_bias,
                      norm_method = norm_method,
                      replace_7x7_conv = replace_7x7_conv,
                      slim_behaviour = slim_behaviour,
...
def get_resnet (resnet_name = ss.RESNET50, 
                use_bias = True, 
                norm_method = None, 
    ...
    stacks = build_stacks(get_resnet_stack_num(resnet_name), 
                          use_bias = use_bias, 
                          norm_method = norm_method, 
                          slim_behaviour = slim_behaviour,
                          custom_block = custom_block)
        
    return ResNet(stacks, use_bias = use_bias,
                          norm_method = norm_method,
                          replace_7x7_conv = replace_7x7_conv, 
                          return_endpoints = return_endpoints,
                          name = resnet_name)
...
def build_stacks (num_of_blocks = [3, 4, 23, 3], 
                  use_bias = True, 
                  norm_method = None, 
                  slim_behaviour = False,
                  custom_block = None):
    ...
    for i in range(4):
        stack = stacks_func(filters = filters_list[i],
