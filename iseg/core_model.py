...
class SegBase (tf.keras.Model):
    ...
    def inference (self, inputs, training = False):
        ...
        results = inference_fn(inputs, model = self, 
    ...
    def inference_with_scale (self, inputs, training = False, 
                                            scale_rate = 1.0, 
                                            flip = False):
        ...
        logits = inference_fn(inputs, model = self, 
                                      num_class = self.num_class, 
                                      training = training, 
                                      sliding_window_crop_size = sliding_window_size) # Under solving  #47261

        logits = convert_to_list_if_single(logits)
        
        logits = multi_results_handler(logits, lambda x : resize_image(x, inputs_size, method = "bilinear", name = "inference_resize_back"))

        logits = multi_results_handler(logits, lambda x : tf.raw_ops.SelectV2(condition = flip, t = tf.image.flip_left_right(x), e = x))
    ...
    def inference_with_multi_scales (self, inputs, training = False, 
                                                   scale_rates = [1.0], 
                                                   flip = False):
        ...
        def loop_body (image, scale_rate = 1.0, flip = False):
            ...
            logits_list = self.inference_with_scale(image, training = training, 
...
class SegFoundation (SegBase):
    ...
    def __init__(self, num_class=21,
        ...
        assert len(aux_loss_rate) == num_aux_loss, (
            f"aux_loss_rate must be scalar or has length = num_aux_loss, found {len(aux_loss_rate)}")

        if num_aux_loss == 0:
            aux_metric_names = None

        assert (aux_metric_names is None) or (len(aux_metric_names) == num_aux_loss),(
    ...
    def custom_losses(self, num_class, 
                            ignore_label, 
                            batch_size, 
                            reduction = False, 
                            **kwargs):
        ...
        loss = lambda post_func: catecrossentropy_ignore_label_loss(num_class = num_class, 
