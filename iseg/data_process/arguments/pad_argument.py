...
class PadArgument(DataArgumentBase):
    ...
    def __init__(self, target_height, 
                       target_width,
                       image_pad_value = [127.5, 127.5, 127.5],
                       label_pad_value = 255,
                       name = None):
    ...
    def call(self, image, label):
        ...
        image = dataprocess.pad_to_bounding_box(image, 0, 0, target_height, target_width,
                                            pad_value = self.image_pad_value)

        if label is not None:

            label = dataprocess.pad_to_bounding_box(label, 0, 0, target_height, target_width, 
