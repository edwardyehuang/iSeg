...
def pad_to_odd (image, label=None, image_pad_value=[127.5, 127.5, 127.5], label_pad_value=255):
    ...
    image = dataprocess.pad_to_bounding_box(
        image, 0, 0, image_height, image_width, pad_value=image_pad_value
    )

    if label is not None:
        label = dataprocess.pad_to_bounding_box(
            label, 0, 0, image_height, image_width, pad_value=label_pad_value
        )
...
class PadToOddAugment(DataAugmentationBase):
    def __init__(
        self, image_pad_value=[127.5, 127.5, 127.5], label_pad_value=255, name=None
    ):
