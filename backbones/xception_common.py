...
class XceptionDepthWiseConv (tf.keras.Model):
    ...
    def __init__(self, block_index, conv_index, filters, strides = (1, 1), activation = False, weight_decay = 0., momentun=0.9):
        ...
        self.depthwise_conv = tf.keras.layers.DepthwiseConv2D((3, 3), strides=strides, padding="same", use_bias=False, name=depthwise_conv_name)
        ...
        self.pointwise_conv = tf.keras.layers.Conv2D(filters, (1, 1), padding="same", use_bias=False, name=pointwise_conv_name)
...
class XceptionBlock (tf.keras.Model):
    ...
    def __init__ (self, block_index, filters_list, strides, skip_connection = 0, activation = False):
        ...
        for i in range (length):
            ...
            self.convs.append(XceptionDepthWiseConv(block_index, i + 1, filters_list[i], strides=conv_strides, activation=activation))
        ...
        if skip_connection == 2:
            ...
            self.shortcut = tf.keras.layers.Conv2D(filters_list[-1], (1, 1), strides=strides, padding="same", use_bias=False, name=shortcut_name)
...
class Xception(tf.keras.Model):
    ...
    def __init__ (self, return_endpoints = False, name = None):
        ...
        self.block1_conv1 = tf.keras.layers.Conv2D(32, (3, 3), strides=(2, 2), padding="same", use_bias=False, name = "block1_conv1")
    ...
    def add_xception_block(self, filters_list, strides, skip_connection = 0, activation = False, repeat = 1):
        ...
        for i in range(repeat):
            self._xception_blocks.append(XceptionBlock(block_index = index + i,
