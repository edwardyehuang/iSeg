...
class BlockType1 (tf.keras.Model):
    def __init__ (self, filters, kernel_size = 3, 
        ...
        if self.conv_shortcut:
            self.shortcut_conv = tf.keras.layers.Conv2D (4 * filters, kernel_size = 1, strides = stride, use_bias = use_bias, name = name + "_0_conv")
            self.shortcut_bn = normalization(epsilon=BN_EPSILON, method = norm_method, name = name + "_0_bn")
        
        self.conv1_conv = tf.keras.layers.Conv2D(filters, kernel_size = 1, strides = stride, use_bias = use_bias, name = name + "_1_conv")
...
class BlockType2 (tf.keras.Model):
    def __init__ (self, filters, kernel_size = 3, 
        ...
        self.conv2_conv = conv2d_same_fn(filters, kernel_size, strides = stride, padding = "SAME", use_bias = use_bias, name = name + "_2_conv")
        self.conv2_bn = normalization(epsilon=BN_EPSILON, method = norm_method, name = name + "_2_bn")
