    ...
    def build(self, input_shape):
        if self.relative_position_embedding_type == '2d_multi_head':
            ...
            if self.scale_ratio is not None:
                if (isinstance(self.scale_ratio, list) and
                        len(self.scale_ratio) == 2):
                    height_scale, width_scale = self.scale_ratio
                elif isinstance(self.scale_ratio, float):
                    height_scale = self.scale_ratio
                    width_scale = self.scale_ratio
                else:
                    raise ValueError(
