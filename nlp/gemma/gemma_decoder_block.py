...
from iseg.nlp.gemma.gemma_attention import CachedGemmaAttention
...
class GemmaDecoderBlock(keras.layers.Layer):
    ...
    def _compute_attention_mask(
        ...
        input_length = output_length = tf.shape(x)[1]
        ...
        if decoder_mask is not None:
            causal_mask = tf.cast(causal_mask, decoder_mask.dtype)
            return tf.minimum(decoder_mask, causal_mask)
