...
from iseg.nlp.gemma.gemma_decoder_block import GemmaDecoderBlock
...
class GemmaBackbone(Backbone):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not (is_keras2_15() or is_keras3()):
            raise ValueError(
                "`GemmaBackbone` requires Keras 3 or 2.5. Run `pip install -U keras` "
        ...
        x = x * tf.cast(tf.sqrt(tf.cast(hidden_dim, tf.float32)), x.dtype)
