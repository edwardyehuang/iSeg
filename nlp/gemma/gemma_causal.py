...
from keras_nlp.src.utils.preset_utils import get_file, load_from_preset


from iseg.nlp.gemma.gemma_backbone import GemmaBackbone
from iseg.nlp.gemma.gemma_causal_lm_preprocessor import (
...
class GemmaCausalLM(GenerativeTask):
    ...
    def call_with_cache(
        ...
        x = x * tf.sqrt(tf.cast(self.backbone.hidden_dim, x.dtype))
    ...
    def _build_cache(self, token_ids):
        ...
        max_length = tf.shape(token_ids)[1]
    ...
    def generate_step(
        ...
        row_lengths = tf.reduce_sum(tf.cast(padding_mask, "int32"), axis=-1)
        # Start at the first index that has no user inputted id.
        index = tf.reduce_min(row_lengths)

        def next(prompt, cache, index):
            # The cache index is the index of our previous token.
            cache_update_index = index - 1
            batch_size = tf.shape(prompt)[0]
            prompt = tf.slice(prompt, [0, cache_update_index], [batch_size, 1])
        ...
        if end_token_id is not None:
            ...
            end_locations = tf.logical_and(
                tf.equal(token_ids, end_token_id),
                tf.logical_not(padding_mask),
            )
            end_locations = tf.cast(end_locations, tf.int32)
            # Use cumsum to get ones in all locations after end_locations.
            cumsum = tf.cast(tf.cumsum(end_locations, axis=-1), tf.int32)
            overflow = cumsum - end_locations
            # Our padding mask is the inverse of these overflow locations.
            padding_mask = tf.logical_not(tf.cast(overflow, tf.bool))
        else:
    ...
    def score(
        ...
        batch_shape = tf.shape(token_ids)[:2]
        assert len(batch_shape) == 2

        if padding_mask is None:
            padding_mask = tf.ones(shape=batch_shape)
        ...
        x = token_embeddings * tf.cast(
            tf.sqrt(self.backbone.hidden_dim), dtype=self.compute_dtype
        )
        for i, transformer_layer in enumerate(self.backbone.transformer_layers):
    ...
    def from_preset(
        ...
        **kwargs,
        ...
        """Instantiate Gemma model from preset architecture and weights.

        Args:
            preset: string. Must be one of "{{preset_names}}".
            load_weights: bool. Whether to load pre-trained weights for the
                model. If `False`, the model will be initialized with random
                weights.

        Returns:
            A `keras_nlp.models.GemmaCausalLM` instance.
        """
